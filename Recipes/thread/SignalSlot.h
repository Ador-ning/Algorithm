...
#ifndef ALGORITHM_SIGNALSLOT_H
#define ALGORITHM_SIGNALSLOT_H
...
namespace Recipes {
	...
	template<typename Callback>
	class SlotImpl : boost::noncopyable {
	public:
...
namespace {
	...
	public:
		...
		typedef Recipes::SignalImpl<Callback> SignalImpl;
		typedef Recipes::SlotImpl<Callback> SlotImpl;
		...
		Slot connect(Callback &&func) {
			boost::shared_ptr<SlotImpl> slotImpl(new SlotImpl(
					impl_, std::forward<Callback>(func)));
			add(slotImpl);
			return slotImpl;
		}

		Slot connect(Callback &&func, boost::shared_ptr<void> &tie) {
			boost::shared_ptr<SlotImpl> slotImpl(new SlotImpl(
					impl_, std::forward<Callback>(func), tie));
			add(slotImpl);
			return slotImpl;
		}
		...
		void call(ARGS &&... args) {
			...
			{
				Recipes::MutexLockGuard lock(impl.mutex_);
				slots = impl_->slots_;
			}
